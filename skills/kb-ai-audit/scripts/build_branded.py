...
import json, re, html, sys, os, argparse, shutil
...
def page(title, extra_css, inner):
    return HEAD.replace("__TITLE__", esc(title)).replace("__EXTRA_CSS__", extra_css) + inner + "</body></html>"

WORDMARK = '<img src="assets/wordmark.png" alt="myAskAI" style="height:%dpx;display:block">'
...
<div style="min-height:100vh;background:#FFFFFF;font-family:var(--font-body);color:#1B1B1B;padding-bottom:56px">
  <div class="wrap" style="padding:26px 32px 0;display:flex;align-items:center;gap:16px;flex-wrap:wrap">
    ...
    <div style="margin-left:auto;display:flex;gap:10px;align-items:center">
      ...
      <button id="copysum" type="button" style="font:500 13.5px var(--font-ui);color:#FBF8F3;background:#EA3609;border:none;border-radius:1000px;padding:10px 17px;cursor:pointer">Copy score summary</button>
    </div>
  </div>
  ...
  <div class="wrap sec-pad" style="padding-top:30px">
    <div style="font:300 12.5px var(--font-body);color:#868181">Every fix works with any AI agent or helpdesk. Keeping a help center AI-ready over time is what <a href="https://myaskai.com" target="_blank" style="color:#EA3609">myAskAI</a> does automatically.</div>
  </div>
...
"""
    (OUT / "dashboard.html").write_text(page(f"{KB} — AI-readiness audit", extra, body))
...
<div style="min-height:100vh;background:#FFFFFF;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;font-family:var(--font-body)">
  <div style="width:1200px;max-width:100%;aspect-ratio:1200/630;background:#FBF8F3;border-radius:24px;box-shadow:0 24px 70px rgba(0,0,0,0.16);padding:52px 58px;display:flex;flex-direction:column;overflow:hidden;position:relative">
    ...
    <div style="display:flex;align-items:center;margin-top:auto;padding-top:18px;border-top:1px solid rgba(0,0,0,0.1);font:300 14px var(--font-body);color:#868181;position:relative">
      ...
      <span style="margin-left:auto;font:600 14px var(--font-ui);color:#EA3609">myaskai.com</span>
    </div>
  </div>
  <div style="font:300 13px var(--font-body);color:#8a857c">Screenshot this card, or right-click &rarr; Save, to share your score.</div>
</div>
"""
    (OUT / "scorecard.html").write_text(page(f"{KB} — AI-readiness score", "", body))
...
<div style="min-height:100vh;background:#FFFFFF;font-family:var(--font-body);color:#1B1B1B;padding-bottom:48px">
  <div style="max-width:820px;margin:0 auto;padding:26px 24px 0">
    ...
    <div style="font:300 12px var(--font-body);color:#868181;margin-top:20px;text-align:center">Rewritten in your own voice. Every fix works with any AI agent or helpdesk &middot; <a href="https://myaskai.com" target="_blank" style="color:#EA3609">myAskAI</a></div>
  </div>
</div>
<script>
(function(){{
...
"""
        (OUT / fns[i]).write_text(page(f'{w.get("new_title")} — {KB}', ART_CSS, body))
    return fns

if __name__ == "__main__":
    build_dashboard(); build_scorecard(); fns = build_rewrites()
    print(f"Branded: dashboard.html, scorecard.html + {len(fns)} rewrite page(s)")
